...
export class RowComparison extends ATouringTask {
  ...
  updateTable() {
    ...
    function updateTableBody(bodyData: IScoreCell[][][], timestamp: string) {
...
function prepareDataArray(colGroups: IAttributeCategory[], rowGroups: IAttributeCategory[], rowAttributes: IColumnDesc[]): any[] {
  ...
  for (const [i, attr] of rowAttributes.entries()) {
    ...
    for (const [j, rowGrp] of rowGroups.entries()) {
      data[i][j] = new Array(colGroups.length + (j === 0 ? 2 : 1)).fill({label: '<i class="fas fa-circle-notch fa-spin"></i>', measure: null} as IScoreCell);
